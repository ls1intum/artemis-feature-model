 ...
 *
 * @param includedFeatures resolved included semantics sorted by candidate id, with the functional members' artifact
 *            mappings resolved by the configuration deriver.
 ...
 * @param configDerivation per-member configuration-key resolutions, or null when conformance failed.
...
public record ModelAssemblyOutcome(List<ResolvedFeatureScope> includedFeatures, CurationReport curation, ManifestConformance conformance,
        FeatureModel generatedModel, ArtemisConfigKeyCatalog generatedCatalog, ConfigDerivationReport configDerivation, boolean generatedOutputConformant,
        boolean modelIntegrityValid, boolean deliveryEligible, List<ReportItem> items) {
}
