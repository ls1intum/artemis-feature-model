 ...
 * deployment environment supplies; this application never invents the value. Unknown JSON fields are a hard parse
 * error, so a mapping authored in the retired {@code valueFromProfile} shape is rejected instead of reinterpreted.
 *
...
public record ArtifactMapping(String target, String path, String source, JsonNode valueWhenSelected, JsonNode valueWhenDeselected, Boolean secret) {
    ...
    /**
     * Normalizes the optional secret flag so an absent JSON field defaults to {@code false}.
     *
     * @param target generated overlay file the entry belongs to.
     * @param path dotted configuration path written into the overlay.
